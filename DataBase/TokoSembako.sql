...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_getPengguna]
	@username varchar(50),
	@pass varchar(50)
AS
BEGIN
	SELECT * FROM Pengguna WHERE username = @username AND pass = @pass
END
GO
...
BEGIN
	INSERT INTO Barang
	VALUES(@id_barang,@id_merk,@id_satuan,@nama,@stok,@harga_jual,@harga_grosir)
END
...
(
	trs_idpj		varchar	(50) NOT NULL PRIMARY KEY,
	id_member		varchar (10) NOT NULL,
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_InputPenjualan]
	@trs_idpj		varchar	(50),
	@id_member		varchar (10),
	@id_pengguna	varchar	(10),
	@tgl_transaksi	date		,
...
--Detail Penjualan--
CREATE TABLE Detailpenjualann
(
	trs_idPj	varchar	(50) NOT NULL,
	...
	FOREIGN KEY (trs_idPj)	REFERENCES Penjualan  (trs_idPj),
	FOREIGN KEY (id_barang) REFERENCES Barang	  (id_barang)
)

SELECT * FROM Detailpenjualann
DROP TABLE DetailPenjualann
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_InputDetailpenjualann]
	@trs_idPj	 varchar	(50) ,
	@id_Barang	 varchar	(10) ,
	@nama		 varchar	(50) ,
	@jumlah		 int,
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[sp_InputPenukaranHadiah]
	@trs_idPH		varchar	(10) ,
	@trs_tanggal	date,
	@id_pengguna	varchar	(10),
	@id_member		varchar	(10),
...
AS
BEGIN
	INSERT INTO PenukaranHadiah VALUES (
		@trs_idPH,
		@trs_tanggal,
		@id_pengguna,
		@id_member,
...
(
	...
	FOREIGN KEY (id_hadiah) REFERENCES HadiahMember		(id_hadiah)
)
...
DROP TABLE DetailTransSupplier
...
CREATE TABLE DetailTransPenjualanr(
   id_detail	varchar	(10) NOT NULL,
   trs_idSP		varchar	(10) NOT NULL,
   id_barang	varchar	(10) NOT NULL,
   nama_barang	varchar	(50) NOT NULL,
   ...
   PRIMARY KEY	(id_detail),
   FOREIGN KEY	(trs_idSP) REFERENCES TransaksiPenjualan(trs_idSP) ON UPDATE CASCADE ON DELETE CASCADE,
   FOREIGN KEY	(id_barang) REFERENCES Barang(id_barang) ON UPDATE CASCADE ON DELETE CASCADE
...
WITH SCHEMABINDING
AS
SELECT id_barang, id_merk, nama, stok, harga_jual, harga_grosir FROM dbo.Barang
GO
...
CREATE VIEW viewSupplier
AS
SELECT tbTransaksiPembelian.pembelian_id AS ID, 
...
AS
	...
	FROM Barang AS P INNER JOIN Detail_TransaksiPenjualan AS T
...
AS
	UPDATE P SET
	stok =(P.stok + T.jumlah)
	FROM Barang AS P INNER JOIN Detail_Transaksisupplier AS T
	ON P.id_barang=T.id_barang
GO
